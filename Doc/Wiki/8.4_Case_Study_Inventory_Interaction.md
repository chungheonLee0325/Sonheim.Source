...
# 8.4. 사례 연구: 클라이언트 예측으로 구현하는 반응형 인벤토리 UX
...
이 문서는 Sonheim의 인벤토리 시스템이 어떻게 **[[클라이언트 예측(Client-Side Prediction)|2.1_Server_Authority_Architecture]]** 기법을 통해 사용자에게는 즉각적인 반응성을 제공하는 동시에, **[[서버 권위(Server Authority)|2.1_Server_Authority_Architecture]]** 아키텍처를 통해 데이터의 정합성을 100% 보장하는지, 그 정교한 상호작용의 전체 흐름을 추적하는 사례 연구입니다.

---

## 8.4.2. 아키텍처: 예측, 요청, 그리고 조정 (Prediction, Request, and Reconciliation)
...
*   **시작점:** 플레이어가 [[`SlotWidget`|7.5_Building_a_Reusable_Inventory_UI]] A를 드래그하여 `SlotWidget` B에 드롭합니다.
*   **흐름:**
...
*   `InventoryWidget`은 [[`InventoryComponent`|4.2_Inventory_System]]의 `SwapItems` 함수를 호출하고, 이 함수는 내부적으로 `ServerSwapItems` RPC를 호출하여 서버에 실제 아이템 스왑을 요청합니다.
...
*   서버로부터 변경된 데이터가 클라이언트에 도착하면, `OnRep_RepItems` 함수가 자동으로 호출됩니다. 이 함수는 서버로부터 받은 최신 데이터로 로컬 캐시(`InventoryItems`)를 재구성하고, [[`OnInventoryChanged` 델리게이트|7.1_Event-Driven_UI_Architecture]]를 다시 한번 브로드캐스트합니다.
...
*   **네트워크 효율성:** 아이템 배열 전체를 보내는 대신, [[`FastArraySerializer`|4.2_Inventory_System]]를 사용하여 변경된 슬롯의 정보만 효율적으로 전송하여 네트워크 부하를 줄입니다.
